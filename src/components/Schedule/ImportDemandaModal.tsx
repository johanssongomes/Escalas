...
interface ImportDemandaModalProps {
  isOpen: boolean;
  onClose: () => void;
  diasCount: number;
  defaultShift: 'T1' | 'T2' | 'T3';
  onApplyDemanda: (shift: 'T1' | 'T2' | 'T3', demandaM3: number[], demandaPcs: number[]) => void;
}

export const ImportDemandaModal: React.FC<ImportDemandaModalProps> = ({
  isOpen,
  onClose,
  diasCount,
  defaultShift,
  ...
  const [selectedShift, setSelectedShift] = useState<'T1' | 'T2' | 'T3'>(defaultShift);
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const isHeader = cols.some(c => {
          ...
          return val.includes('dia') || val.includes('m³') || val.includes('m3') || val.includes('pcs') || val.includes('pçs') || val.includes('peça') || val.includes('volume') || val.includes('demanda');
  ...
  const downloadTemplate = () => {
    ...
    const exampleRows = Array.from({ length: diasCount }, () => {
      return `${400 + Math.round(Math.random() * 100)};${1000 + Math.round(Math.random() * 200)}`;
    }).join('\n');
    ...
    link.setAttribute('download', `modelo_importacao_demanda_${selectedShift}.csv`);
  ...
  const handleApply = () => {
    ...
    onApplyDemanda(selectedShift, m3Array, pcsArray);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-extrabold text-slate-800 dark:text-slate-100">
              Importar Demanda de Turno (m³ e Peças)
            </h3>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto space-y-4">
          ...
          {/* Shift Selection */}
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-950/40 p-3 rounded-2xl border border-slate-100 dark:border-slate-800">
            <label className="text-xs font-black text-slate-500 uppercase tracking-wider shrink-0">
              Turno Destino:
            </label>
            <select
              value={selectedShift}
              onChange={(e) => setSelectedShift(e.target.value as 'T1' | 'T2' | 'T3')}
              className="text-xs font-black bg-white dark:bg-slate-900 border border-slate-250 dark:border-slate-800 rounded-xl px-3 py-1.5 focus:ring-1 focus:ring-violet-500 text-slate-700 dark:text-slate-200"
            >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full py-2 bg-slate-800 hover:bg-slate-900 dark:bg-slate-750 dark:hover:bg-slate-700 text-white font-bold text-xs rounded-xl transition disabled:opacity-50 cursor-pointer"
                >
          ...
          {status === 'preview' && (
            <div className="space-y-4">
              <div className="p-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/60 rounded-2xl flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-emerald-800 dark:text-emerald-400">
                    Dados mapeados com sucesso para o {selectedShift === 'T1' ? '1º Turno (T1)' : selectedShift === 'T2' ? '2º Turno (T2)' : '3º Turno (T3)'}!
                  </h4>
              ...
              <div className="flex gap-3 justify-end pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                >
                  Aplicar Dados ao {selectedShift}
                </button>
              </div>
            </div>
          )}
